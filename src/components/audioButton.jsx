...
const AudioButton = ({
  ...
  // After getting mediaRecorder from useAudioRecorder
  useEffect(() => {
    // Update mediaRecorderRef with the mediaRecorder instance from useAudioRecorder
  ...
  useEffect(() => {
    ...
    // Now mediaRecorderRef.current should correctly reference the MediaRecorder instance
    const stream = mediaRecorderRef.current.stream; // Access the stream from the current MediaRecorder
    const audioCtx = new (window.AudioContext || window.webkitAudioContext)();
    const source = audioCtx.createMediaStreamSource(stream);
    analyserRef.current = audioCtx.createAnalyser();
    analyserRef.current.fftSize = 256;
    ...
    drawFrequencyBars(); // Call the function to start drawing the frequency bars
    ...
    return () => {
      ...
      if (analyserRef.current) {
        analyserRef.current.disconnect(); // Disconnect the analyser
      }
    };
  }, [isRecording, mediaRecorderRef.current]); // Depend on `isRecording` and the current MediaRecorder instance
      ...
      >
        {isRecording ? (
          ...
          <div className='flex items-center justify-between h-full w-full'>
            ...
            <div className="relative h-[50px] w-full mx-2" onClick={(e) => { e.stopPropagation(); handleWaveformClick(e);}}>
              <canvas ref={waveformRef} className="absolute top-0 left-0 w-full h-full"></canvas>
              <canvas ref={dotCanvasRef} className="absolute top-0 left-0 w-full h-full"></canvas>
            </div>
          ...
          <div className='flex items-center justify-between w-full'>
            ...
            <span className='pl-3 text-sm font-normal'>{Math.floor(timer / 60)}:{("0" + (timer % 60)).slice(-2)}</span>
          </div>
        )}
      </button>
    </div>
